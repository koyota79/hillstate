    ...
    <div v-for="(item ,index) in contents"  :style="item.offset" :key="index" class="collapsible" :class="[{active :item.isActive }, `menu_${index}`]" :ref="getContentRef(item.id)" >
            ...
            <transition name="expand" @enter="enter" @after-enter="afterEnter" @leave="leave" >
                    <div class="content"  :class="`content_${index}`" v-if="item.isActive">
                        <div v-for="(shopObj ,s_index) in item.shop_list" class="table"  style="display: table;">
                           <div class="tr" style="display: table-row;">
                                <div  class="td" style="" @click="selectedShop($event , shopObj)">
                                    {{shopObj.shop_nm}}
                                </div>
                           </div> 
                        </div>
            ...
            contents: [
                {
                    ...
                    title: '가',
                    shop_list: [
                         {id :0 ,shop_nm : '거북이 한의원'}
                        ,{id :1 ,shop_nm : '1111111'}
                        ,{id :2 ,shop_nm : '22222'}
                        ,{id :3 ,shop_nm : '33333'}
                        ,{id :4 ,shop_nm : '4444'}
                    ],
